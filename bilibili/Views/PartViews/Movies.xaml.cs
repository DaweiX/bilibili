    ...
    public sealed partial class Movies : Page
    {
        ...
        private async void Pivot_SelectionChanged(object sender, SelectionChangedEventArgs e)
        {
            ...
            switch (pivot.SelectedIndex)
            {
                case 0:
                    {
                        ...
                        {
                            var temp = await ContentServ.GetContentAsync(23, 1);
                ...
                case 1:
                    {
                        if (gv_short.Items.Count == 0)
                        {
                            var temp = await ContentServ.GetContentAsync(85, 1);
                            foreach (var item in temp)
                            {
                                gv_short.Items.Add(item);
                            }
                        }
                    }
                    break;
                case 2:
                    {
                        if (gv_ur.Items.Count == 0)
                        {
                            var temp = await ContentServ.GetContentAsync(145, 1);
                            foreach (var item in temp)
                            {
                                gv_ur.Items.Add(item);
                            }
                        }
                    }
                    break;
                case 3:
                    {
                        if (gv_jp.Items.Count == 0)
                        {
                            var temp = await ContentServ.GetContentAsync(146, 1);
                            foreach (var item in temp)
                            {
                                gv_jp.Items.Add(item);
                            }
                        }
                    }
                    break;
                case 4:
                    {
                        if (gv_ch.Items.Count == 0)
                        {
                            var temp = await ContentServ.GetContentAsync(147, 1);
                            foreach (var item in temp)
                            {
                                gv_ch.Items.Add(item);
                            }
                        }
                    }
                    break;
                case 5:
                    {
                        if (gv_other.Items.Count == 0)
                        {
                            var temp = await ContentServ.GetContentAsync(83, 1);
                            foreach (var item in temp)
                            {
                                gv_other.Items.Add(item);
                            }
                        }
                    }
                    break;
