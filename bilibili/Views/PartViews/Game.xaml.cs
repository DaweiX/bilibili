    ...
    public sealed partial class Game : Page
    {
        ...
        private async void Pivot_SelectionChanged(object sender, SelectionChangedEventArgs e)
        {
            ...
            switch (pivot.SelectedIndex)
            {
                case 0:
                    {
                        ...
                        {
                            var temp = await ContentServ.GetContentAsync(4, 1);
                ...
                case 1:
                    {
                        if (gv_common.Items.Count == 0)
                        {
                            var temp = await ContentServ.GetContentAsync(17, 1);
                            foreach (var item in temp)
                            {
                                gv_common.Items.Add(item);
                            }
                        }
                    }
                    break;
                case 2:
                    {
                        if (gv_online.Items.Count == 0)
                        {
                            var temp = await ContentServ.GetContentAsync(65, 1);
                            foreach (var item in temp)
                            {
                                gv_online.Items.Add(item);
                            }
                        }
                    }
                    break;
                case 3:
                    {
                        if (gv_musicgame.Items.Count == 0)
                        {
                            var temp = await ContentServ.GetContentAsync(136, 1);
                            foreach (var item in temp)
                            {
                                gv_musicgame.Items.Add(item);
                            }
                        }
                    }
                    break;
                case 4:
                    {
                        if (gv_mugen.Items.Count == 0)
                        {
                            var temp = await ContentServ.GetContentAsync(19, 1);
                            foreach (var item in temp)
                            {
                                gv_mugen.Items.Add(item);
                            }
                        }
                    }
                    break;
                case 5:
                    {
                        if (gv_gmv.Items.Count == 0)
                        {
                            var temp = await ContentServ.GetContentAsync(121, 1);
                            foreach (var item in temp)
                            {
                                gv_gmv.Items.Add(item);
                            }
                        }
                    }
                    break;
