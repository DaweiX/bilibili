...
namespace bilibili.Views.PartViews
{
    ...
    /// </summary>
    public sealed partial class Dance : Page
    {
        public Dance()
        {
        ...
        private async void Pivot_SelectionChanged(object sender, SelectionChangedEventArgs e)
        {
            ...
            switch (pivot.SelectedIndex)
            {
                case 0:
                    {
                        ...
                        {
                            var temp = await ContentServ.GetContentAsync(20, 1);
                ...
                case 1:
                    {
                        if (gv_dance.Items.Count == 0)
                        {
                            var temp = await ContentServ.GetContentAsync(154, 1);
                            foreach (var item in temp)
                            {
                                gv_dance.Items.Add(item);
                            }
                        }
                    }
                    break;
                case 2:
                    {
                        ...
                        {
                            var temp = await ContentServ.GetContentAsync(156, 1);
                            foreach (var item in temp)
                            {
