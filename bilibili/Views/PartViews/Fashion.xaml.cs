...
namespace bilibili.Views.PartViews
{
    ...
    /// </summary>
    public sealed partial class Fashion : Page
    {
        public Fashion()
        {
        ...
        private async void Pivot_SelectionChanged(object sender, SelectionChangedEventArgs e)
        {
            ...
            switch (pivot.SelectedIndex)
            {
                case 0:
                    {
                        ...
                        {
                            var temp = await ContentServ.GetContentAsync(155, 1);
                ...
                case 1:
                    {
                        if (gv_makeup.Items.Count == 0)
                        {
                            var temp = await ContentServ.GetContentAsync(157, 1);
                            foreach (var item in temp)
                            {
                                gv_makeup.Items.Add(item);
                            }
                        }
                    }
                    break;
                case 2:
                    {
                        if (gv_cloth.Items.Count == 0)
                        {
                            var temp = await ContentServ.GetContentAsync(158, 1);
                            foreach (var item in temp)
                            {
                                gv_cloth.Items.Add(item);
                            }
                        }
                    }
                    break;
                case 3:
                    {
                        if (gv_exe.Items.Count == 0)
                        {
                            var temp = await ContentServ.GetContentAsync(164, 1);
                            foreach (var item in temp)
                            {
                                gv_exe.Items.Add(item);
                            }
                        }
                    }
                    break;
                case 4:
                    {
                        ...
                        {
                            var temp = await ContentServ.GetContentAsync(159, 1);
                            foreach (var item in temp)
                            {
